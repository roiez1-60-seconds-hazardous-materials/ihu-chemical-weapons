...
const dict: Record<string, Record<Lang, string>> = {
  ...
  'summary.p1': { he: 'איראן פיתחה וחימשה חמ"ת — חומרים מבוססי תרופות באמצעות תוכנית מחקר בת שני עשורים שמרכזה באוניברסיטת אימאם חוסיין (IHU) של משמרות המהפכה.', en: 'Iran has developed and weaponized pharmaceutical-based agents (PBAs) through a two-decade research program centered at Imam Hossein University (IHU), the IRGC\'s sole academic institution.' },
  'summary.p2': { he: 'תוכנית החמ"ת מסמלת מעבר מנשק השמדה המונית לנשק הגורם לאיבוד הכרה ומוות — כלים המיועדים לשימוש יומיומי במלחמות פרוקסי באזור האפור.', en: "Iran's PBA program represents a paradigm shift from WMDs to tactical incapacitation weapons — tools designed for everyday use in proxy wars in the gray zone." },
